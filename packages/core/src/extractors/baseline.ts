import { DocumentText } from "../ingest/types";
import type { ExtractionResult, FieldResult, Span } from "..";

function findSpan(text: string, value: string): Span[] {
  if (!value) return [];
...
export function baselineExtract(doc: DocumentText, _opts: BaselineOptions = {}): ExtractionResult {
  const text = doc.text || "";
  ...
  if (m) {
    ...
    if (m) {
      const sym = m[1];
      const map: Record<string, string> = { "$": "USD", "€": "EUR", "£": "GBP", DT: "TND", TND: "TND" };
  ...
  for (const line of lines) {
    ...
    if (cells.length >= 2) {
      ...
      if (numsInLine >= 2) rows.push(cells);
    }
  }
  if (rows.length) {
