...
function mergeResults({ perChunk, schemaFields }) {
    const fields = {};
    const warnings = [];
...
function earliestSpan(fr) {
    if (!fr.spans || fr.spans.length === 0)
        return Number.MAX_SAFE_INTEGER;
    return Math.min(...fr.spans.map((s) => s.start));
}
function criticalConfidence(fields, schemaFields) {
    const crits = schemaFields.filter((f) => f.critical);
    if (crits.length === 0)
        return avg(Object.values(fields).map((f) => f.confidence));
    return avg(crits.map((f) => fields[f.name]?.confidence ?? 0));
}
function avg(nums) {
    if (nums.length === 0)
        return 0;
