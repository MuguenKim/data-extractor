...
function looksLikeCSVorTSV(buf: Buffer): boolean {
  const s = buf.toString('utf8');
  return /(,|\t).+\n/.test(s);
}

export function ingestSpreadsheet(buf: Buffer, opts: IngestOptions): DocumentText {
  ...
  if (looksLikeCSVorTSV(buf)) {
    ...
    try {
      ...
      const lines: string[] = [];
      const notes: string[] = [];
      wb.SheetNames.forEach((name: string, si: number) => {
